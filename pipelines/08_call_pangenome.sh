#!/bin/bash
# Sentieon Pangenome DNAscope - WGS mode on the shared dedup BAM.
...
CALLER="pangenome"
TIMEFILE="${LOG_DIR}/${COV}x/time/pangenome.time"
OUTPUT_VCF=""
PANGENOME_MODEL_BUNDLE=""
CMD=()

sentieon_skip_if_no_license "Pangenome DNAscope"
...
OUTPUT_VCF="${OUT_DIR}/SS_${CHR_TO_USE}_PANGENOME_${COV}x_WGS.vcf.gz"
...
echo "=== Running Sentieon Pangenome DNAscope (WGS, shared BAM) ==="

/usr/bin/time -v -o "${TIMEFILE}" "${CMD[@]}"

sentieon_log_metrics "pangenome" "Pangenome_DNAscope" "${TIMEFILE}"

echo
echo "Pangenome DNAscope done: ${OUTPUT_VCF}"
